...
NHL.regression <- NHL.train %>%
  select(Salary, Ht, Wt, Hand, DftRd, Born, G, A1, DftYr, dzFOL, Cntry, GP, Position, SA)
...
mod1.cv <- fit_resamples(lm.workflow,
  resamples = NHL.cv6, 
  metrics = metric_set(mae, rsme)#SPECIFY HERE?
) %>%
...
lasso_wf_tune <- workflow() %>% 
  add_recipe(lm.recpie) %>% # recipe defined above

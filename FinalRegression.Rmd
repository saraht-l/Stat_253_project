...
lm_spec <-
    ...
    set_engine(engine = 'lm') %>% 
    set_mode('regression')
baby_model<- fit(lm_spec,Salary~Wt+DftRd+G+A1+DftYr+GP+SA,data=NHL.regression2)
baby_model%>% tidy()
...
mod1.cv <- fit_resamples(lm.workflow,
  resamples = NHL.cv6, 
  metrics = metric_set(mae, rmse, rsq)#SPECIFY HERE?
) %>%
...
model_wf <- workflow() %>%
  add_formula(Salary~Wt+DftRd+G+A1+DftYr+GP+SA) %>%
  add_model(lm_spec)

mod2.cv <- fit_resamples(model_wf,
  resamples = NHL.cv6, 
  metrics = metric_set(rmse, rsq, mae)
)
mod2.cv%>%collect_metrics()


mode3_wf <- workflow() %>%
  add_formula(Salary~Wt+DftRd+G) %>%
  add_model(lm_spec)

mod3.cv <- fit_resamples(mode3_wf,
  resamples = NHL.cv6, 
  metrics = metric_set(rmse, rsq, mae)
)
mod3.cv%>%collect_metrics()
```
...
best_penalty2 <- select_best(tune_output, metric = 'mae') #different penalty value based on the mae
Credit_final_wk <- finalize_workflow(lasso_wf_tune, best_penalty) # incorporates penalty value to workflow
Credit_final_wk2<-finalize_workflow(lasso_wf_tune, best_penalty2) 
...
tune_output %>% collect_metrics() %>% filter(penalty == (best_penalty2 %>% pull(penalty)))#metrics for second lasso model
```

#Visualize redisuals 
```{r}
#Evaluate whether some quantitative predictors might be better modeled with nonlinear relationships
mod2.cvoutput<- mod2.cv%>%
  predict(new_data=NHL.cv6)%>%
  bind_cols(NHL.cv6)%>%
  mutate(resid= Salary-.pred)
ggplot(mod2.cvoutput, aes(x = .pred, y=resid)) +
  geom_point() +
  geom_smooth() +

...
NHL.regression2 <- NHL.regression %>%  
  transform(Cntry,Country=as.numeric(factor(Cntry))) %>% mutate(rookie=as.factor(ifelse(NHL.regression$DftYr>2013,1,0)))%>%
...
heart_rf_OOB_output2 %>% 
  roc_curve(success, 1, event_level = "second") %>% autoplot()

heart_rf_OOB_output2 %>% 
  roc_auc(success, 1, event_level = "second") #Area under Curve
```

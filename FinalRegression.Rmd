...
ggplot(NHL.regression2, aes(x=dzFOL, y=Salary)) +
 ...
 ggplot(NHL.regression2, aes(x=Country, y=Salary)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_smooth(method = "lm", color = "red", se = FALSE) +
    theme_classic()
...
#THIS CODE CHUNK IS BROKEN !!!!
#### WE ALSO NEED TO DECIDE WHICH ONES ARE LINEAR AND WHICH ONES WE WANT TO ADD SPLINES TO ####
...
# Original Recipe
lm_rec <- recipe(Salary ~ Ht + Wt + DftRd + G + A1 + DftYr + dzFOL + Country + GP + SA, data = NHL.regression2) %>%
...
ns_mod <- fit(
  splines_wf, #workflow
  data = NHL.regression2
)

```
```{r}

#### THIS CODE WON'T WORK UNTIL WE GET THE CODE ABOVE TO WORK ###
#### WE ALSO NEED TO EDIT THIS CODE TO VISUALIZE THE VARIABLES AGAINST THE PREDICTORS FOR ONLY THE VARIABLES WE DECIDE TO IMPLEMENT A SPLINE ON ####

spline_mod_output <- ns_mod %>%
    predict(new_data = NHL.regression2) %>% #generates a column of predictions 
    bind_cols(NHL.regression2) %>% #takes what you are inputting and binds it to something else
    mutate(resid = Salary - .pred)
...
ggplot(spline_mod_output, aes(x = Country, y = resid)) +
    geom_point() +
    geom_smooth() +
    geom_hline(yintercept = 0, color = "red") +
    theme_classic()
